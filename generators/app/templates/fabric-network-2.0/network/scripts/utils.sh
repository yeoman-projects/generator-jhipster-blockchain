...
chaincodeAdd() {
  ...
  parsePeerConnectionParameters $@
  res=$?
  verifyResult $res "Add transaction failed on channel '$CHANNEL_NAME' due to uneven number of peer and org parameters "
  CCARGS='{"Args":["add","'$HASHTOSET'"]}'
  ...
  cat log.txt
  verifyResult $res "Add execution on $PEERS failed "
  echo "===================== Add transaction successful on $PEERS on channel '$CHANNEL_NAME' ===================== "
  echo
}
...
chaincodeQueryHash() {
  ...
  do
    ...
    echo "Attempting to Query peer${PEER}.org${ORG} ...$(($(date +%s) - starttime)) secs"
    set -x
    peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["query","'$HASHTOTEST'"]}' >&log.txt
    res=$?
    set +x
...
readAll() {
  PEER=$1
  ORG=$2
  echo "===================== reading all the ledger on channel '$CHANNEL_NAME'... ==================="
  echo "Attempting to Query peer${PEER}.org${ORG} ...$(($(date +%s) - starttime)) secs"
  set -x
  peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["readAll"]}' >&log.txt
  res=$?
  set +x
  echo
  cat log.txt
